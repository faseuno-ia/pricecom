...
import { evaluateRunHealthGate, type RunHealthGateResult } from "../../lib/catalog/run-health-gate";
...
export async function runPartialCommitShadow(deps: PartialCommitShadowDeps): Promise<PartialCommitShadowReport> {
  ...
  if (!partition.partitionCoversCatalog) {
    ...
    return buildReport("STOP_PARTITION_DOES_NOT_COVER_CATALOG", "DIAGNOSTIC_ONLY", false, false, partition, reconciled.providerDiscovery.providerNewSkuCount,
      ...
      evaluatePricePreflight({ priceWriteSet: partition.priceWriteSet, presentWithoutPriceCount: partition.presentWithoutPriceCount }), lifecycle, 0);
  }
  ...
  if (!decision.authorizeWrite) {
    ...
    await deps.markCompletedNoWrite({ totalProducts: partition.totalCatalogRows, productsWithPrice: 0, productsWithoutPrice: partition.presentWithoutPriceCount, productsWithoutSku: 0 });
    return buildReport(decision.classification, decision.lifecyclePreviewStatus, false, false, partition, reconciled.providerDiscovery.providerNewSkuCount, health, preflight, lifecycle, 0);
  }
  ...
  const report = buildReport(decision.classification, decision.lifecyclePreviewStatus, true, fenced.committed, partition, reconciled.providerDiscovery.providerNewSkuCount, health, preflight, lifecycle, fenced.writtenCount);
